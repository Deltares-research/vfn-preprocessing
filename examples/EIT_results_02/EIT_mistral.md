...
$\Delta T_{L}$ : temperatuurverschil van het effluent met de achtergrond/verschil tov inname [ ${ }^{\circ} \mathrm{C}$ ]. iv, 25-27, 29
...
$\Phi_{\text {atm }}$ : warmteflux met atmosfeer $\left[W / \mathrm{m}^{2}\right] .26$
...
Ad 4: Een zelfde definitie als voor getijdewateren wordt gehanteerd, maar de dimensies vanThe image is a technical diagram illustrating a cross-sectional view of a road or similar structure. The diagram includes several key elements: a horizontal roadway labeled 'Eb' with a flood level indicated by 'Vlood' above it. The width of the road is denoted by 'B'. There is a circular area with a radius 'R' that is five times the width of the road (R=5*B) and a maximum of 500 meters. The circle intersects the road at a height of 0.25B above the base of the road. The diagram also shows a shaded region within the circle, indicating an area of interest or impact, and a point within this region where specific calculations or considerations apply, represented by the ratio (V_vlood/V_eb) * R. The diagram likely represents a scenario involving flood levels and their impact on the road structure.het ontvangende water waar de mengzone op wordt gebaseerd zijn de dimensies en andere karakteristieken van de geul bij laagwater. Als de lozing tijdens laag water op een drooggevallen plaat plaatsvindt, dan zullen andere criteria gehanteerd worden. Daar wordt in de definitie van de mengzone geen uitspraak over gedaan.
...
Dit figuur komt uit IJff (2000)The image contains a detailed technical diagram and explanation in Dutch regarding the behavior of effluent discharge into a water system. It outlines the conditions and calculations for determining whether the discharge initially forms a jet or a plume, and the subsequent transition to a 2D plume. The diagram includes formulas for calculating the maximum distance of the jet and the concentration of the effluent in both 3D and 2D plumes. It also provides conditions for different scenarios, such as when the water depth or the bed of the water surface is a limiting factor. The legend explains the variables and coefficients used in the formulas, and default values for certain coefficients are provided.# 3.3 Jet3D 
...
$$The image depicts a schematic diagram of a system involving fluid flow and heat transfer. The diagram includes a rectangular block with various annotations indicating different types of flows and heat interactions. Key elements include: Qx, representing a heat flow into the system; Φatm, indicating atmospheric interaction; QL and ΔC, representing fluid flow rates and concentration changes; and ΦL, indicating another form of heat interaction. The arrows illustrate the direction of these flows, with some entering and exiting the block, and others circulating within the system. The diagram likely represents a process involving both thermal and fluid dynamics, possibly in an engineering or environmental context.
...
# 9.4.3 Recirculatie in andere waterenThe image depicts a schematic diagram of a thermal system with three main components, each labeled with a temperature difference (ΔT0, ΔT1, and ΔT2). The system appears to illustrate the flow of heat and energy through these components. Here is a detailed breakdown: 1. The first component receives an input heat flow [Qa] and has an atmospheric heat loss (Φatm). The heat flow through this component is labeled Qx, and it results in a temperature difference ΔT0. 2. The second component receives the output from the first component, which is the heat flow Qx and the temperature difference ΔT0. It also has an atmospheric heat loss (Φatm) and produces a new temperature difference ΔT1. 3. The third component receives the output from the second component, which is the heat flow Qx and the temperature difference ΔT1. It also has an atmospheric heat loss (Φatm) and produces a new temperature difference ΔT2. Additionally, there is a feedback loop involving a heat flow QL. This heat flow is split into two paths: one that adds ΔT0 to the temperature and another that adds ΔT + ΔTL to the temperature. These paths then combine and feed back into the system with a temperature difference of ΔT0 + ΔTL. The diagram illustrates the complex interactions and flow of heat through the system, highlighting the temperature differences and heat losses at each stage.# 9.4.3.1 Recirculatie in andere wateren met complexere waterbeweging 
...
De eerste stap voor het uitvoeren van de berekeningen is het selecteren van een locatie en dat gebeurt door in te zoomen en met de muis te klikken op een locatie (zoals hieronder al is gedaan). Selecteren kan alleen wanneer de segmenten zichtbaar worden, dus er moet voldoende worden ingezoomed, zie figuur A.2.The image is a screenshot of a web-based map interface titled 'Immissietoets.' The map appears to be focused on a specific geographic region, likely in the Netherlands, as indicated by the shape of the land and the presence of Dutch place names. The map includes various layers that can be toggled on and off, such as 'Stofbelasting' (Particle loading) and 'Temperatuurloading' (Temperature loading). The map also shows water bodies and possibly other environmental or geographical features. There is a prompt on the right side of the screen that says 'Start immissietoets' and instructs the user to click on the map to select a location. The interface includes navigation tools and options to zoom in and out of the map.![img-3.jpeg](img-3.jpeg)
...
5 klik op de knop "resultaten". Een basisberekening wordt uitgevoerd en de beslisboom wordt zichtbaar onder de invoerdata.The image is a map showing the discharge of a substance named 'Immissietoets' into a river system. The map includes various layers such as locations, water bodies, and measurement points. The right side of the image contains detailed information about the discharge, including the name, code, segment, type of discharge, and specific substances being discharged. The map highlights the areas affected by the discharge and provides options to view additional information and zoom in for more detailed views.De naam van het waterlichaam verschijnt als KRW Waterlichaam. Er kan gekozen worden voor een ander waterlichaam als daar specifieke redenen voor zijn.
...
3 de ligging van de pijp in de horizontaal als de verticaal (hier aan de oever en aan het oppervlak). Hier is een beperkte keuze voor beschikbaar via een drop-down lijst met de default 'Oever' en 'Oppervlak'.The image displays a detailed technical report in Dutch, focusing on water discharge calculations and effluent characteristics. Key parameters include water depth (9.73 m), average flow velocity (0.623 m/s), surface and bottom temperatures (both 18.2°C), surface and bottom salinities (0.336 and 0.343 PSU respectively), and various flow rates and quality metrics. The report also includes sections on effluent discharge, location specifics, and substance concentrations, with a final section on the mixing zone length (2569.57 m).Wanneer geen gebruiker gedefinieerde afstand wordt opgegeven wordt de toetsafstand afgeleid van de dimensies van het ontvangende water (hier niet ingevuld). Voor een ingevuld scherm zie figuur A. 4
...
Druk op de knop "geavanceerde berekening" en een geavanceerde berekening wordt uitgevoerd en genereert een beslisboom zoals hierboven in figuur A. 4 aangegeven. In de uitvoer zijn nog een aantal knoppen zichtbaar: Drinkwater inname: Wanneer op deze knop wordt gedrukt dan verschijnt de volgende tabel met de uitkomsten van de drinkwatertoets (figuur A.5):The image depicts a decision tree diagram used for evaluating the compliance of a discharge with various regulatory standards. The diagram includes several steps and conditions to determine if the discharge meets the required norms. The key components include: 1. Effluent Test (Ce <= JG-MKN): This initial step checks if the effluent concentration (Ce) is less than or equal to the annual average environmental quality norm (JG-MKN). 2. Significance Test (ΔCI <= 0.1 * JG-MKN): This step checks if the change in concentration (ΔCI) is within an acceptable range. 3. Norm Test JG (ΔCI + Cw <= JG-MKN): This step combines the change in concentration with the background concentration (Cw) to see if it meets the JG norm. 4. Norm Test MAC (ΔCI + Cw > MAC): This final step checks if the combined concentration exceeds the Maximum Allowable Concentration (MAC). The diagram also includes a legend explaining the variables and terms used. The results indicate that the discharge does not meet the MAC norm, as the concentration at the MAC test point is 6.108 µg/l, which is above the threshold.Resultaten
...
| Locatie | Concentratie verhooging [ug/l] | Achtergrondconcentratie [ug/l] | Totale concentratie [ug/l] | Volstant aan norm |
| :-- | :-- | :-- | :-- | :-- |
| Rotterdam, Reijervosarij, Nive Maas | 0.263 | 0.006 | 0.268 | Ja |
...
Er is ook een log uitvoer waarin in detail kan worden gekeken naar details van berekeningsresultaten. Deze is met name bedoeld om in detail te onderzoeken welkeThe image is a graph that shows the concentration of a substance (in micrograms per liter) as a function of distance from a release point (in meters). The x-axis represents the distance from the release point, ranging from 0 to 700 meters. The y-axis represents the concentration, ranging from 0 to 100 micrograms per liter. The graph shows a steep decline in concentration as the distance from the release point increases, with the concentration dropping sharply within the first 100 meters and then leveling off to near zero beyond that point. There are two data series represented by different colors and markers: one with solid lines and another with dashed lines. The legend on the right indicates that the data series correspond to different conditions or scenarios, labeled as 'MKN (0.6)'.getallen zijn gebruikt en voor expert analyse als dat nodig is. Met de knop kan een pdf uitvoer worden gegenereerd waarin alle in en uitvoer van de berekeningen zijn opgenomen. Deze kan lokaal op een computer worden opgeslagen.
...
Na het uitvoeren van de berekening wordt het volgende scherm zichtbaar. Zaak is om te controleren dat alle velden inderdaad aanwezig zijn.The image shows a map with temperature measurements of a river. The map includes various layers such as discharge segments, informative layers, and temperature measurements. The river is highlighted in blue, and there are specific points marked with yellow icons indicating the start of temperature measurements. The right side of the image contains detailed information about the temperature measurements, including the location, length, and other relevant data. The text on the right provides additional context and instructions related to the temperature measurements.![img-8.jpeg](img-8.jpeg)
...
Figuur A.9: Indicatie van het gebied dat door de koudelozing wordt beïnvloedThe image displays a technical diagram related to the temperature calculation of cold water in a plumbing system. The diagram is divided into several sections, including input parameters, calculations, and graphical results. On the left side, various parameters such as flow rate, temperature, and length of the pipe are inputted. The middle section shows detailed calculations and intermediate results. On the right side, there are two graphs depicting the temperature change over distance for both the plumbing flow and the ambient environment. The graphs illustrate how the temperature of the water changes as it flows through the pipe, with different lines representing the temperature of the flow, ambient temperature, and allowable temperature differences. The bottom section includes a summary of the results and a note indicating that the calculation has been successfully completed.The image shows a map with the title 'Resultaten NWM segmenten' at the top, indicating the results of NWM (possibly a hydrological or water management model) segments. The map displays a segment of the Nieuwe Maas river, with a color gradient bar at the bottom ranging from red to blue, representing different values or measurements. The blue line across the map likely represents the river's flow or a specific measurement within the river segment. The map includes various geographical features such as roads, buildings, and water bodies. The color gradient bar at the bottom indicates a range of values from -0.0 to -4.0, with the blue line corresponding to the -4.0 value. The map is part of a larger interface with tabs for different views or data, including 'Resultaten,' 'Beslisboom,' 'Kaart,' 'Logs,' and 'PDF.'# A. 4 Informatie 
...
Deze link verwijst naar: https://iplo.nl/contact/vragenformulier/ en deze link kan dan ook worden getest of inderdaad naar een bestaand adres wordt verwezen.The document discusses the legal and technical aspects of assessing and managing emissions into surface water. It emphasizes that the application does not have legal status and that the results should not be used for well-founded assessments. The text outlines the requirements for assessing emissions, including the need for a clear determination of the location and sources of emissions, and the use of emission reduction measures. It also mentions specific regulations and guidelines, such as the 'Besluit Emissiebesluiten' and the 'Besluit Toelating Stoffen', and provides information on how to assess the impact of emissions on water quality. The document includes a section on assessing temperature increases from cooling systems and the effects of multiple discharges. It concludes by stating that cumulative effects of multiple discharges are not addressed in the assessment.